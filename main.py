...
def list(msg):
    ...
    def check2(reaction, user):
        if reaction.message.id != msg.id or user == client.user:
            return False
        if reaction.emoji == "✔️":
            exercises.add(msg)
    ...
    async def on_reaction_add(reaction, user):
        if user == "Gym_Bot#6486":
            ...
            if reaction.emoji == "🏋️" and user != "Gym_Bot#6486":
                pages = []
                await msg.delete()
                embedVar = discord.Embed(title=f"Gym", colour=0xb042f5)
                embedVar.add_field(
                    name="Biceps",
                    value=
                    "1. Seated Hammer Curls\n2. Bicep Curls\n3. EZ Bar Curls\n4. Preacher Curls\n5. Hammer Curl \n6. Chin Ups \n7. Dumbell Incline Bicep Curls"
                )
                embedVar2 = discord.Embed(title=f"Gym", colour=0xb042f5)
                embedVar2.add_field(
                    name="Triceps",
                    value=
                    "1. Dips\n2. Rope Pushdowns\n3. Skullcrushers\n4. Overhead Dumbell Extensions"
                )
                embedVar3 = discord.Embed(title=f"Gym", colour=0xb042f5)
                embedVar3.add_field(
                    name="Shoulders",
                    value=
